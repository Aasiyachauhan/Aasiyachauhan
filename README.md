# Hi, I'm Aasiya Chauhan

**Cybersecurity Graduate | SOC Analyst**

Seeking opportunities in SOC operations, incident response, and cloud security.

## Education
**Honours Bachelor of Information Sciences (Cyber Security)**
Sheridan College (2023–2026)

## Certifications
- CompTIA Security+
- Microsoft SC-200
- OCI Architect Associate
- OCI Foundations Associate

## Work Experience
**IT Security Analyst – Descartes Systems Group**
- Monitored SIEM, EDR, UEBA, and DLP alerts
- Investigated phishing, malware, and security incidents
- Supported threat hunting and incident response

## Featured Projects
- **AI-Based Phishing Detection System**  
  ML-powered phishing detection with Microsoft Graph API.
  https://github.com/Sheridan-Group8/AI-Based-Phishing-Detection-System-with-URL-Analysis
  
- **IOC Scanner**  
  Python tool for analyzing IPs, URLs, domains, and file hashes using VirusTotal.
  https://github.com/Aasiyachauhan/IOC-Scanner

## Technical Skills 

### Security & Operations 
- SIEM, UEBA, EDR, DLP
- Threat Intelligence & Security Monitoring
- Incident Response & Investigation
- Alert Triage & Log Analysis
-
### Cloud Security 
- Azure, AWS, Oracle Cloud (OCI)
- Identity & Access Management (IAM)
- Policy Enforcement & Cloud Security Controls

### Network Security 
- Firewalls, VPNs, ACLs
- IDS/IPS, Traffic Inspection
- Network Segmentation
-
### Programming & Scripting 
- Python
- Bash
- PowerShell
-
### Operating Systems 
- Linux (Kali, Ubuntu) 
- Windows 10/11
- Windows Server

## Focus Areas 
- SOC Operations
- Threat Hunting 
- Digital Forensics 
- Cloud Security 
- Security Automation

## 📫 Connect
**LinkedIn:** www.linkedin.com/in/aasiya-chauhan
